...
class MatchingLayer(nn.Module, FromParams):
    ...
    def forward(self, context_p, mask_p, context_h, mask_h):
        ...
        # (batch, seq_len, 1)
        mask_p_3d, mask_h_3d = mask_p.unsqueeze(-1), mask_h.unsqueeze(-1)
        # (batch, seq_len, 1, 1)
        mask_p_4d, mask_h_4d = mask_p_3d.unsqueeze(-1), mask_h_3d.unsqueeze(-1)

        # explicitly set masked weights to zero
        context_p = context_p * mask_p_3d
        context_h = context_h * mask_h_3d
        ...
        # (batch, seq_len*, 1)
        cosine_max_p = masked_max(cosine_sim, mask_p_3d, dim=2, keepdim=True)
        cosine_mean_p = masked_mean(cosine_sim, mask_p_3d, dim=2, keepdim=True)
        cosine_max_h = masked_max(cosine_sim.permute(0, 2, 1), mask_h_3d, dim=2, keepdim=True)
        cosine_mean_h = masked_mean(cosine_sim.permute(0, 2, 1), mask_h_3d, dim=2, keepdim=True)
        ...
        if not self.wo_maxpool_match:
            ...
            mv_p_max = masked_max(mv_max, mask_p_4d, dim=2)
            mv_p_mean = masked_mean(mv_max, mask_p_4d, dim=2)

            # (batch, seq_len2, seq_len1, num_perspective)
            mv_max = mv_max.permute(0, 2, 1, 3)

            # (batch, seq_len2, num_perspective)
            mv_h_max = masked_max(mv_max, mask_h_4d, dim=2)
            mv_h_mean = masked_mean(mv_max, mask_h_4d, dim=2)
        ...
        if not self.wo_max_attentive_match:
            # (batch, seq_len*, hidden_size)
            att_max_h = masked_max(att_h, mask_p_4d, dim=2)
            att_max_p = masked_max(att_p.permute(0, 2, 1, 3), mask_h_4d, dim=2)
