...
class MaskedMultiHeadSelfAttention(Seq2SeqEncoder):
    ...
    def forward(self,  # pylint: disable=arguments-differ
        ...
        expanded_mask = mask.repeat(1, num_heads).view(batch_size * num_heads, timesteps)
...
class SelfAttentiveLstm(Seq2SeqEncoder):
    ...
    def forward(self,  # pylint: disable=arguments-differ
        ...
        self_attention_encoded_inputs = self._self_attention_encoder(lstm_encoded_inputs, mask)

        # the "enhancement" layer
        enhanced = torch.cat(
            [inputs, lstm_encoded_inputs, self_attention_encoded_inputs],
            dim=-1
        )
